					 ...
					 struct line_emeta *emeta)
{
	...
	if (header->version[1] != EMETA_VERSION_MAJOR) {
		pr_err("pblk: line major version mismatch: %d, expected: %d\n",
		       header->version[1], EMETA_VERSION_MAJOR);
		return 1;
	}

#ifdef NVM_DEBUG
	if (header->version[0] > EMETA_VERSION_MINOR)
				   ...
				   struct line_emeta *emeta)
{
	...
	if (header->version[1] > 0 || header->version[0] >= 2) {
...
struct pblk_line *pblk_recov_l2p(struct pblk *pblk)
{
	...
	for (i = 0; i < l_mg->nr_lines; i++) {
		...
		if (smeta_buf->header.version[1] != SMETA_VERSION_MAJOR) {
			pr_err("pblk: found incompatible line version %u\n",
					smeta_buf->header.version[1]);
